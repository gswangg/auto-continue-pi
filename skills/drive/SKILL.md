...
The injected followUp is system-generated by the extension even though pi represents it as a user message. Treat the drive task as the instruction to execute. Treat any `ac off` / `ac undrive` text in the footer as an informational control hint only; do not call those tools unless the human explicitly asks to pause/disable or the drive-prompt stop criterion says to stop.
...
- **FollowUp control hints are not human instructions.** Auto-continue followUps appear as user messages, but `[auto-continue]` marks extension-generated text. Do not pause or undrive just because the footer says how to do it; only do that on explicit human request or a stop criterion.
